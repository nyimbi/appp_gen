...
def gen_models(metadata, inspector, output_file='models.py'):
                ...
                if isinstance(col["type"], Enum):
                    ...
                    for index, fk in enumerate(fks):
                        if col['name'] == fk["constrained_columns"][0]:
                            ...
                            pkey = fk["referred_table"] + "." + fk["referred_columns"][0]
...
def gen_views(metadata, inspector, output_file='views.py'):
    with open(output_file, 'w') as f:
...
#     )
@click.option(
        "-d", "--database", default="postgresql:///wakala", help="The name of the database to introspect"
    )
# @click.option(
